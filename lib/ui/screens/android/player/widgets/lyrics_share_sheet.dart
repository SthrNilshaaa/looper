...
import 'package:looper_player/features/playback/presentation/widgets/advanced_lyric_line.dart' show artworkColorProvider;
import 'package:looper_player/ui/widgets/app_bottom_sheet.dart';
import 'package:looper_player/ui/widgets/lyrics_share_card.dart';
...
class _LyricsShareSheetContent extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<_LyricsShareSheetContent> createState() => _LyricsShareSheetContentState();
}

class _LyricsShareSheetContentState extends ConsumerState<_LyricsShareSheetContent> {
  ...
  Future<void> _onShare() async {
    ...
    try {
      ...
      final safeTitle = widget.song.title.replaceAll(RegExp(r'[^a-zA-Z0-9_\-]'), '_');
      final file = File(
      ...
      await Share.shareXFiles(
        [XFile(file.path)],
        text: '${widget.song.title} — ${widget.song.artist ?? "Unknown Artist"}',
      );
    } finally {
  ...
  Future<void> _onSaveToGallery() async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Could not save the image to the gallery.')),
        );
      }
    } finally {
        ...
        children: [
          Row(
            children: [
              Text(
                'Share Lyrics',
                style: AppFonts.jostStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.bold),
              ),
              const Spacer(),
              IconButton(
          ...
          Align(
            ...
            child: Text(
              'Background Color',
              style: AppFonts.jostStyle(color: Colors.white54, fontSize: 12.5, fontWeight: FontWeight.w600),
            ),
          ),
...
class _ColorSwatchButton extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final markColor = color.computeLuminance() > 0.5 ? Colors.black : Colors.white;
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          border: selected ? const Border.fromBorderSide(BorderSide(color: Colors.white, width: 2)) : null,
        ),
        child: DecoratedBox(
          decoration: BoxDecoration(
            shape: BoxShape.circle,
          ...
          child: selected ? Icon(LucideIcons.check, color: markColor, size: 16) : null,
        ),
      ),
    );
  }
