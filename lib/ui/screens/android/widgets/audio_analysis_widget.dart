...
class _AudioAnalysisCardState extends State<AudioAnalysisCard> {
  ...
  Future<void> _analyze({bool forceRefresh = false}) async {
    ...
    try {
      ...
      if (image == null && data.spectrum != null && data.spectrum!.sliceCount > 0) {
  ...
  static Future<void> _saveToCache(String filePath, AudioAnalysisData data) async {
    try {
      final dir = await _cacheDir();
      final key = _cacheKey(filePath);
      final file = File('${dir.path}/$key.json');
  ...
  static Future<void> _saveSpectrogramToCache(String filePath, ui.Image image) async {
    try {
      final dir = await _cacheDir();
      final key = _cacheKey(filePath);
...
class _AudioInfoCard extends StatelessWidget {
            ...
            Wrap(
              ...
              children: [
                ...
                _MetricChip(
                  ...
                  label: "Clipping",
                  value: _formatClipping(data.clippingSamples),
                  cs: cs,
                ),
  ...
  String _formatChannels(AudioAnalysisData data) {
    ...
    if (data.channels == 2) return "Stereo";
    if (data.channels == 1) return "Mono";
  ...
  String _formatClipping(int samples) {
    if (samples <= 0) return "None (Clean)";
