...
class _EditAlbumSheetState extends ConsumerState<EditAlbumSheet> {
  ...
  Future<void> _resetArtworkToDefault() async {
    ...
    try {
      ...
      if (pictureData != null && pictureData.isNotEmpty) {
        final newPath = await LibraryScanner().saveAlbumArt(widget.album.name, pictureData);
        if (!mounted) return;
  ...
  Future<void> _save() async {
    ...
    if (mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(success ? l10n.albumInfoUpdated : l10n.failedToSaveChanges),
                ...
                children: [
                  ...
                  Center(
                    child: Wrap(
                      ...
                      children: [
                        TextButton.icon(
                          onPressed: _isResettingArt ? null : _resetArtworkToDefault,
